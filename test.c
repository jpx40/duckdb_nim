...
VV_LOCAL_SYMBOL  int compare_14877176222143175693_RepIndex_by_idx(RepIndex* a, RepIndex* b) {
...
VNORETURN VV_LOCAL_SYMBOL void panic_debug(int line_no, string file, string mod, string fn_name, string s) {
	...
	#else
	{
		...
		eprintln( str_intp(2, _MOV((StrIntpData[]){{_SLIT("   v hash: "), 0xfe10, {.d_s = _SLIT("7baff15")}}, {_SLIT0, 0, { .d_c = 0 }}})));
		eprintln(_SLIT("========================================="));
		#if defined(_VNATIVE)
		{
			exit(1);
...
string string_replace_each(string s, Array_string vals) {
	...
	if (idxs.len > 0) { qsort(idxs.data, idxs.len, idxs.element_size, (int (*)(const void *, const void *))&compare_14877176222143175693_RepIndex_by_idx); };
	u8* b = malloc_noscan((int)(new_len + 1));
	int idx_pos = 0;
...
string string_trim(string s, string cutset) {
	...
	multi_return_int_int mr_45080 = string_trim_indexes(s, cutset);
	int left = mr_45080.arg0;
	int right = mr_45080.arg1;
